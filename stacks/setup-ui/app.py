...
def generate_config():
    ...
    try:
        ...
        with open(ENV_FILE, 'w') as f:
            ...
            pihole_password = security.get('pihole_password')
            if pihole_password:
                ph = PasswordHasher()
                pihole_hash = ph.hash(pihole_password)
                f.write(f"PIHOLE_PASSWORD_HASH={pihole_hash}\n")
                f.write("# NOTE: Only the hash of the password is stored here for security.\n")
                f.write("# For Pi-hole configuration, securely retrieve the password as needed.\n")
            else:
                f.write("PIHOLE_PASSWORD_HASH=CHANGE_ME_REQUIRED\n")
            f.write("PIHOLE_DNS1=127.0.0.1#5335\n")
            f.write("PIHOLE_DNS2=127.0.0.1#5335\n")
            f.write("# WEBPASSWORD: Set in Pi-hole securely, retrieve plain password via secure workflow\n")
            f.write("\n")
            f.write("# Grafana\n")
