...
message="${timestamp} [BACKUP] VIP=${VIP_ADDRESS:-} IFACE=${NETWORK_INTERFACE:-} NODE_ROLE=${NODE_ROLE:-} ROUTER_ID=${ROUTER_ID:-} TYPE=${TYPE} NAME=${NAME} STATE=${STATE}"
...
keepalived_vrrp_state{job="${job}",instance="${instance}",router_id="${router_id}",name="${NAME}",type="${TYPE}",state="${STATE}"} 0
EOF
fi
