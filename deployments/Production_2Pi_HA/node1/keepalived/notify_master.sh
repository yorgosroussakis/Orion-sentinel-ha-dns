...
# =============================================================================
#
# This script is called when the node transitions to MASTER state.
# The VIP is now active on this node.
#
# =============================================================================
...
send_signal_notification() {
    ...
    local message="🟢 DNS HA: ${HOSTNAME} is now MASTER\nVIP ${VIP} is active on this node\nTime: ${TIMESTAMP}"
...
EOF
    ...
    echo "${TIMESTAMP}" > "${state_dir}/master_since"
    rm -f "${state_dir}/backup_since" "${state_dir}/fault_since" 2>/dev/null || true
}

# Main
main() {
    log "🟢 MASTER: ${HOSTNAME} entered MASTER state for ${VRRP_INSTANCE}"
    log "   VIP ${VIP} is now active on this node"
    ...
    if [[ "${NOTIFY_ON_FAILOVER:-true}" == "true" ]]; then
        local message="DNS HA: ${HOSTNAME} is now MASTER"
        local details="VIP ${VIP} is active. Priority: ${PRIORITY}"
